...
class InviteTracker(commands.Cog):
    ...
    # ================= 📥 মেম্বার ও বট জয়েন ট্র্যাকিং লজিক =================
    @commands.Cog.listener()
    ...
    async def addinvite(self, ctx, member: discord.Member, amount: int):
        ...
        if uid not in config["invite_data"][gid]:
            config["invite_data"][gid][uid] = {"regular": 0, "fake": 0, "leave": 0, "bonus": 0, "bots": 0}
            
        config["invite_data"][gid][uid]["bonus"] += amount
        save_config(config)
        
        embed = discord.Embed(description=f"<:Star:1472268505238863945> সফলভাবে **{amount}** বোনাস ইনভাইট {member.mention}-কে দেওয়া হয়েছে।", color=get_theme_color(ctx.guild.id))
    ...
    @commands.hybrid_command(name="resetinvite", description="🗑️ নির্দিষ্ট মেম্বারের ডাটা রিসেট করুন")
    @commands.has_permissions(administrator=True)
    async def resetinvite(self, ctx, member: discord.Member):
        config = load_config()
        gid, uid = str(ctx.guild.id), str(member.id)
        ...
        embed = discord.Embed(description=f"<:dot:1472268394391670855> {member.mention}-এর ইনভাইট ডাটা রিসেট করা হয়েছে।", color=get_theme_color(ctx.guild.id))
        embed.set_author(name=f"Action by {ctx.author.name}", icon_url=ctx.author.display_avatar.url)
        await ctx.send(embed=embed)

    # ================= ⚠️ ৩. পুরো সার্ভার রিসেট (Reset All) =================
    @commands.hybrid_command(name="resetallinvite", description="⚠️ পুরো সার্ভারের সবার ডাটা রিসেট করুন")
    @commands.has_permissions(administrator=True)
    async def resetallinvite(self, ctx):
        config = load_config()
        if "invite_data" in config and str(ctx.guild.id) in config["invite_data"]:
            config["invite_data"][str(ctx.guild.id)] = {}
            save_config(config)
            
        embed = discord.Embed(description=f"<:dot:1472268394391670855> পুরো সার্ভারের সব ইনভাইট ডাটা সফলভাবে রিসেট করা হয়েছে!", color=discord.Color.red())
        embed.set_author(name=f"Action by {ctx.author.name}", icon_url=ctx.author.display_avatar.url)
        await ctx.send(embed=embed)
