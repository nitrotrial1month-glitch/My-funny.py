...
def wishlist():
    ...
    """
# বটকে ওয়েবসাইটের সাথে লিংক করার জন্য গ্লোবাল ভেরিয়েবল 
nova_bot = None 

# --- 🚀 Discord DM Function for Sellers (Thread-Safe) ---
def notify_sellers_via_dm(order_id, name, phone, address, items, payment_method):
    if not nova_bot or not nova_bot.loop:
        print("Nova bot is not attached to the web server.")
        return
    ...
    async def send_dms():
        ...
        for seller_id, s_items in seller_items.items():
            try:
                # সেলারের ডিসকর্ড আইডি ফেচ করা
                user = await nova_bot.fetch_user(int(seller_id))
                if not user:
                    continue
                
                # মেসেজের ডিটেইলস তৈরি করা (ছবি ছাড়া)
                item_details = ""
                for i in s_items:
                ...
                # সেলারকে সরাসরি DM পাঠানো
                await user.send(message_content)
                print(f"Successfully sent DM to seller {seller_id}")
                
            except Exception as e:
                print(f"Failed to send DM to seller {seller_id}: {e}")

    # ওয়েবসাইট থ্রেড থেকে বটের অ্যাসিনক্রোনাস লুপে কাজ পাঠানো
    asyncio.run_coroutine_threadsafe(send_dms(), nova_bot.loop)
